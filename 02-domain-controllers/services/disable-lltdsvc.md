...
* **GPO Path / Registry Location**:
  ...
  * **Registry Location**: `HKLM\SYSTEM\CurrentControlSet\Services\lltdsvc\Start`

---

## Rationale
Domain Controllers represent the highest privilege tier (Tier 0) in the Active Directory forest. Minimizing the execution footprint on these critical servers is an essential hardening guideline. Disabling the Link-Layer Topology Discovery Mapper (lltdsvc) service directly supports this:
...
4. Create a new Registry Preference (Right-click **Registry -> New -> Registry Item**):
   ...
   * **Key Path**: `SYSTEM\CurrentControlSet\Services\lltdsvc`
   * **Value name**: `Start`
...
[Download Script: Configure-Disablelltdsvc.ps1](../implementation_scripts/Configure-Disablelltdsvc.ps1)
...
[Download Script: Get-lltdsvcStatus.ps1](../audit_scripts/Get-lltdsvcStatus.ps1)
