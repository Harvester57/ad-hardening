...
## Rationale
Domain Controllers represent the highest privilege tier (Tier 0) in the Active Directory forest. Minimizing the execution footprint on these critical servers is an essential hardening guideline. Disabling the Xbox Live Auth Manager (XblAuthManager) service directly supports this:
...
4. Create a new Registry Preference (Right-click **Registry -> New -> Registry Item**):
   ...
   * **Key Path**: `SYSTEM\CurrentControlSet\Services\XblAuthManager`
   * **Value name**: `Start`
