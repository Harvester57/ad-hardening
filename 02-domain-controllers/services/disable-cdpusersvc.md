...
* **GPO Path / Registry Location**:
  ...
  * **Registry Location**: `HKLM\SYSTEM\CurrentControlSet\Services\CDPUserSvc\Start`

---

## Rationale
Domain Controllers represent the highest privilege tier (Tier 0) in the Active Directory forest. Minimizing the execution footprint on these critical servers is an essential hardening guideline. Disabling the Connected Devices Platform User Service (CDPUserSvc) service directly supports this:
...
4. Create a new Registry Preference (Right-click **Registry -> New -> Registry Item**):
   ...
   * **Key Path**: `SYSTEM\CurrentControlSet\Services\CDPUserSvc`
   * **Value name**: `Start`
...
[Download Script: Configure-DisableCDPUserSvc.ps1](../implementation_scripts/Configure-DisableCDPUserSvc.ps1)
...
[Download Script: Get-CDPUserSvcStatus.ps1](../audit_scripts/Get-CDPUserSvcStatus.ps1)
