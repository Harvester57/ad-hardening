...
* **GPO Path / Registry Location**:
  ...
  * **Registry Location**: `HKLM\SYSTEM\CurrentControlSet\Services\MapsBroker\Start`

---

## Rationale
Domain Controllers represent the highest privilege tier (Tier 0) in the Active Directory forest. Minimizing the execution footprint on these critical servers is an essential hardening guideline. Disabling the Downloaded Maps Manager (MapsBroker) service directly supports this:
...
4. Create a new Registry Preference (Right-click **Registry -> New -> Registry Item**):
   ...
   * **Key Path**: `SYSTEM\CurrentControlSet\Services\MapsBroker`
   * **Value name**: `Start`
...
[Download Script: Configure-DisableMapsBroker.ps1](../implementation_scripts/Configure-DisableMapsBroker.ps1)
...
[Download Script: Get-MapsBrokerStatus.ps1](../audit_scripts/Get-MapsBrokerStatus.ps1)
