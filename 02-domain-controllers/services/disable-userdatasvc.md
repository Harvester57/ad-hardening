...
* **GPO Path / Registry Location**:
  ...
  * **Registry Location**: `HKLM\SYSTEM\CurrentControlSet\Services\UserDataSvc\Start`

---

## Rationale
Domain Controllers represent the highest privilege tier (Tier 0) in the Active Directory forest. Minimizing the execution footprint on these critical servers is an essential hardening guideline. Disabling the User Data Access (UserDataSvc) service directly supports this:
...
4. Create a new Registry Preference (Right-click **Registry -> New -> Registry Item**):
   ...
   * **Key Path**: `SYSTEM\CurrentControlSet\Services\UserDataSvc`
   * **Value name**: `Start`
...
[Download Script: Configure-DisableUserDataSvc.ps1](../implementation_scripts/Configure-DisableUserDataSvc.ps1)
...
[Download Script: Get-UserDataSvcStatus.ps1](../audit_scripts/Get-UserDataSvcStatus.ps1)
