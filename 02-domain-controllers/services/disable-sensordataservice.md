...
* **GPO Path / Registry Location**:
  ...
  * **Registry Location**: `HKLM\SYSTEM\CurrentControlSet\Services\SensorDataService\Start`

---

## Rationale
Domain Controllers represent the highest privilege tier (Tier 0) in the Active Directory forest. Minimizing the execution footprint on these critical servers is an essential hardening guideline. Disabling the Sensor Data Service (SensorDataService) service directly supports this:
...
4. Create a new Registry Preference (Right-click **Registry -> New -> Registry Item**):
   ...
   * **Key Path**: `SYSTEM\CurrentControlSet\Services\SensorDataService`
   * **Value name**: `Start`
...
[Download Script: Configure-DisableSensorDataService.ps1](../implementation_scripts/Configure-DisableSensorDataService.ps1)
...
[Download Script: Get-SensorDataServiceStatus.ps1](../audit_scripts/Get-SensorDataServiceStatus.ps1)
