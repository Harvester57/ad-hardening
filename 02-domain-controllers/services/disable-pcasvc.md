...
* **GPO Path / Registry Location**:
  ...
  * **Registry Location**: `HKLM\SYSTEM\CurrentControlSet\Services\PcaSvc\Start`

---

## Rationale
Domain Controllers represent the highest privilege tier (Tier 0) in the Active Directory forest. Minimizing the execution footprint on these critical servers is an essential hardening guideline. Disabling the Program Compatibility Assistant Service (PcaSvc) service directly supports this:
...
4. Create a new Registry Preference (Right-click **Registry -> New -> Registry Item**):
   ...
   * **Key Path**: `SYSTEM\CurrentControlSet\Services\PcaSvc`
   * **Value name**: `Start`
...
[Download Script: Configure-DisablePcaSvc.ps1](../implementation_scripts/Configure-DisablePcaSvc.ps1)
...
[Download Script: Get-PcaSvcStatus.ps1](../audit_scripts/Get-PcaSvcStatus.ps1)
