...
## Rationale
To minimize the attack surface of standard client endpoints and member servers, all unnecessary system services must be disabled. Disabling the Computer Browser (Browser) service directly supports this:
...
* **Verification**: Administrators must verify that client operations do not rely on local Computer Browser capabilities before domain-wide enforcement.
