...
## Rationale
To minimize the attack surface of standard client endpoints and member servers, all unnecessary system services must be disabled. Disabling the Infrared monitor service (irmon) service directly supports this:
...
* **Verification**: Administrators must verify that client operations do not rely on local Infrared monitor service capabilities before domain-wide enforcement.
