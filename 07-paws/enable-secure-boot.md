...
* **BIOS Mode Conversion**: Systems running in legacy BIOS mode (Compatibility Support Module - CSM) instead of Native UEFI cannot use Secure Boot. Converting these systems requires changing partition styles from MBR to GPT (using tools like `MBR2GPT.exe`) and changing firmware settings; refer to [REQ-PAW-005 - UEFI Firmware Security Hardening](configure-uefi-security.md) for firmware settings. Improper conversion can cause boot failures if not executed correctly.
